...
import jzxy.cbq.server.mapper.AccountMapper;
import jzxy.cbq.server.service.AccountService;
import jzxy.cbq.server.utils.Const;
import jzxy.cbq.server.utils.FlowUtils;
  ...
  /**
    ...
    public void createSubAccount(CreateSubAccountVO vo) {
        ...
        account = new Account(null, vo.getUsername(), passwordEncoder.encode(vo.getPassword()),
                vo.getEmail(), Const.ROLE_NORMAL,Const.DEFAULT_AVATAR, new Date(), JSONArray.copyOf(vo.getClients()).toJSONString());
        this.save(account);
    }
